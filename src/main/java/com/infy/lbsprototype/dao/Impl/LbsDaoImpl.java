...
import com.infy.lbsprototype.model.ResultsJSON;
...
public class LbsDaoImpl extends HibernateDaoSupport implements LbsDao {
	...
	@Override
	public GetMyNewsStoriesJsonData getMyNewsStoriesDAO(String customerId, String regionCode) {
		log.info("=============before Get news Stories available for thr given user===================");
		GetMyNewsStoriesJsonData getMyNewsStories = null;
		try{
			List<Customer> customerList = getHibernateTemplate().find("from Customer where customerId='"+customerId+"'");
			log.info("===========customer Id: "+customerId);
			if(!customerList.isEmpty()){
				log.info("=======customer is register Customer==========");
				List<Notification> notifications = getHibernateTemplate().find("from Notification where customerId = '"+customerId+"' and movementType='movein' order by notificationTime desc");
				Notification notification = notifications.get(0);
				if(regionCode.equalsIgnoreCase(notification.getRegionCode())){
					log.info("=======customer Notification Details : "+notification.getRegionCode());
					getMyNewsStories = new GetMyNewsStoriesJsonData();
				}else{
					return getMyNewsStories;
				}
				
				List<Message> messagesList = getHibernateTemplate().find("from Message where topicid in (select preference from CustomerPreferenceMapping where customerid='"+customerId+"' and categoryId='NEWS') and geolocation='"+notification.getRegionCode()+"' and contentTypeId='PER'");
				List<ResultsJSON> resultsJSONList = new ArrayList<ResultsJSON>();
				getMyNewsStories.setStatus("OK");
				getMyNewsStories.setCopyright("Infosys LBS News Solution");
				getMyNewsStories.setNumOfResults(messagesList.size());
				for (Message message : messagesList) {
					ResultsJSON resultsJSON = new ResultsJSON();
					resultsJSON.setTopic(message.getTopicId());
					resultsJSON.setTitle(message.getTitle());
					resultsJSON.setMsgId(message.getMsgid());
					resultsJSON.setCategory(message.getCategoryId());
					resultsJSON.setGeoLocation(message.getGeolocation());
					resultsJSON.setSource(message.getSources());
					resultsJSON.setPublishDate(message.getActiveFrom().toString());
					resultsJSON.setImageUrl(message.getImgUrl());
					resultsJSONList.add(resultsJSON);
				}
				log.info("===sizi of resultsJSONLIst: "+resultsJSONList.size());
				getMyNewsStories.setResultsJSONList(resultsJSONList);
			}
		}catch(Exception e){
			log.info("----------"+e.getMessage()+e.getClass());
		}
